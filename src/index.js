...
window.addEventListener('click', ()=> openPanel(-1));

btns.forEach((item, key) => {
  item.addEventListener('click', (event) => openPanel(key, event));
});

function openPanel(index, event) {
  // 防止冒泡事件，不會向外傳遞
  if (event) {
...
panels.forEach((item) => {
  item.addEventListener('click', (event) => stopPropagation(event));
});
